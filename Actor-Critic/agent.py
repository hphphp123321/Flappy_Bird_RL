...
class Agent():
    def __init__(self, action_set):
        ...
        self.model, self.modelT, self.modelPi = self.init_netWork()
        self.greedy = 0
        self.action_set = action_set
        self.mse = nn.MSELoss()
        self.lr_rate = 3e-4
        self.optimizer = optim.Adam(params=self.model.parameters(), lr=self.lr_rate)
        self.optimizerPi = optim.Adam(params=self.modelPi.parameters(), lr=self.lr_rate)
        self.train_step = 0
    ...
    def init_netWork(self):
        ...
        :return:
        """
        model1 = QNet(in_dim=3, out_dim=2)
        model2 = QNet(in_dim=3, out_dim=2)
        model3 = PiNet(in_dim=3, out_dim=2)
        # model = torch.nn.DataParallel(model, device_ids=[0, 1, 2])
        return model1.to(device=self.device), model2.to(device=self.device), model3.to(device=self.device)

    def train_model_q(self, E):
        ...
        return (old_state_q[action] - state_q[action]).data


    def train_model_pi(self, qt, E):
        cur_state, action, r, next_state, next_action, done = E[0]
        cur_state = torch.tensor(cur_state, dtype=torch.float32).to(self.device)
        action_p = self.modelPi(cur_state)
        print(action_p)
        log_prob = torch.log(action_p[action])

        self.optimizerPi.zero_grad()
        actor_loss = -log_prob*qt
